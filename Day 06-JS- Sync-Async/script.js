...
//Check the nationality of the person:

function checkNationality() {
    var name = document.getElementById("getName").value.trim();
    if (!name) {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (response && response.country && response.country.length > 0) {
                var html = "<strong>Predicted Nationalities for: " + response.name + "</strong><ul>";
                response.country.forEach(function(c) {
...
function fetchDog() {
    fetch("https://dog.ceo/api/breeds/image/random")
        ...
        .then(data => {
            if (data.status === "success") {
                document.getElementById("dogResult").innerHTML = `
                    <img src="${data.message}" alt="Random Dog" style="max-width:300px; border-radius:8px;">
                `;
            } else {
        ...
        .catch(error => {
            document.getElementById("dogResult").innerText = "Error: " + error;
        });
}
